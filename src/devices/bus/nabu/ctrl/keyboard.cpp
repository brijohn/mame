...
void nabu_keyboard_device::nabu_kb_mem(address_map &map)
{
	...
	map(0x7000, 0x7000).w(FUNC(nabu_keyboard_device::adc_start_w));
	map(0x9000, 0x9000).w(FUNC(nabu_keyboard_device::cpu_ack_irq_w));
	map(0xB000, 0xB700).w(FUNC(nabu_keyboard_device::adc_latch_w));
	map(0xD000, 0xD000).r(FUNC(nabu_keyboard_device::adc_data_r));
...
ioport_constructor nabu_keyboard_device::device_input_ports() const
{
...
void nabu_keyboard_device::irq_w(uint8_t data)
{
	if (data && !m_eoc) {
...
void nabu_keyboard_device::adc_start_w(offs_t offset, uint8_t data)
{
	m_adc->start_w(1);
	m_adc->start_w(0);
}

void nabu_keyboard_device::cpu_ack_irq_w(offs_t offset, uint8_t data)
{
